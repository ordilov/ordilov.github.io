...
const info_container_link = c4, info_container_link_hover = c3, info_container_link_active = c5,
    description = c1 + 'cc';
const tag_link = c4, tag_link_border = c5, tag_link_hover = c2, tag_link_hover_background = c3,
    ...
    return (
        <div className='card-layout'>
            {postsMetaData.map((metadata, index) => {
                    ...
                    const imgType = metadata.metadata.imgName ? 'image/' + metadata.metadata.imgName.split('.').pop() : null;
                    return (
                        <div className='card-container' key={metadata.metadata.title}>
                            <div>
                                <div className='img-container'>
                                    {metadata.metadata.imgName &&
                                        <img src={'/images/' + metadata.metadata.imgName} alt={metadata.metadata.title}/>}
                                </div>

                                <div className='info-container'>
                                    <div className='description-container'>
                                        <Link href={`/blog/${metadata.metadata.id}`}
                                    ...
                                    <div className='tags-container'>
                                        <div className='tags-container-tag'>
                                            {metadata.tags && metadata.metadata.tags.map((tag_name, index) => {
                                                return (
